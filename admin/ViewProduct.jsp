...
<head>
    ...
    <title>DASHBOARD | View Product</title>
						...
						<div class="box">
							...
							<div class="box-body table-responsive no-padding">
								<table class="table table-hover"id="productTable">
									...
									<%
										ResultSet rs=table.show("Select * from product");
										while(rs.next())
										{
											%>
												<tr>
													...
													<td><%= rs.getInt(8) %></td>
													<td><%= rs.getInt(9) %></td>
													<td><img src="../images/product/<%= rs.getString(10) %>" height="150"/></td>
													<td><%= rs.getString(11) %></td>
													<td><a href="UpdateProductDetail.jsp?productId=<%= rs.getInt(1) %>&&pageUrl=ViewProduct.jsp"><button type="button" class="btn btn-primary"><span class="fa fa-edit"></span></button></a></td>
													<td><a href="RemoveProductDB.jsp?productId=<%= rs.getInt(1) %>&&pageUrl=ViewProduct.jsp"><button type="button" class="btn btn-danger"><span class="fa fa-remove"></span></button></td>
												</tr>
											<%
										}
									%>
	...
	<script>
		function search()
		{
				...
				for(i=0;i<tr.length;i++)
				{
					...
					if(td)
					{
						if(td.innerHTML.toUpperCase().indexOf(filter)>-1 || tdId.innerHTML.toUpperCase()==filter)
						{
							tr[i].style.display="";
						}
						else
